...
import { FieldsAsAssetIdToPmsCopyComponent } from './fields-as-asset-id/fields-as-asset-id.component';
import { AddInstructionToPmsCopyComponent } from './add-instruction/addInstruction.component';
import { PmsService } from 'src/app/pages/pms/pms.service';
import { environment } from 'src/environments/environment';

@Component({
  selector: 'edit-instructionPMS-copy',
  templateUrl: './editInstruction.component.html',
  styleUrls: ['./editInstruction.component.scss'],
  providers: [treeService],
})
export class EditInstructionToPmsCopyComponent implements OnInit {
  ...
  addInstruction() {
    ...
    const dialogRef = this.dialog
      .open(AddInstructionToPmsCopyComponent, {
        width: '40vw',
        data: {},
        disableClose: true,
      })
  ...
  LinkWithFieldInAsset(Assets: any, fieldType: any, dataInst: any) {
    ...
    dialogRef.afterClosed().subscribe((result) => {
      if (result) {
        const FieldDialogRef = this.dialog.open(FieldsAsAssetIdToPmsCopyComponent, {
          width: '50vw',
  ...
  confirmdeleteFileInSop(fileData: any) {
    this.selectedItemToDelete = fileData?.ID;
