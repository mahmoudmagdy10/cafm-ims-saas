...
export class ppmTasksService {
  ...
  isShedule: BehaviorSubject<any> = new BehaviorSubject<any>(false);
  isShedule$: Observable<any> = this.isShedule.asObservable();
  //  Code
  ...
  getAllTask(params: any) {
    ...
    this.http
      .getData(
        this.isShedule?.value
          ? '/PreventiveMaintenance/UpComingTimeSchedules'
          : '/WorkOrder/list',
        {
          LocationId: localStorage.getItem('defaultLocation'),
          ...params,
      ...
      .subscribe((value) => {
        ...
        if (!this.workOrderCompleted) {
          this.http
            .getData(
              this.isShedule.value
                ? '/PreventiveMaintenance/UpComingTimeSchedulesListByUser'
                : '/WorkOrder/ListUser',
              {
                LocationId: localStorage.getItem('defaultLocation'),
                ...params,
    ...
    (!this.isShedule.value
      ? this.http
          .getData('/WorkOrder/CalenderList', {
            ...
            FromDueDate: this.paramsFilter.FromDueDate
              ? this.paramsFilter.FromDueDate
              : this.FromDueDateForCalender,

            ToDueDate: this.paramsFilter.ToDueDate
              ? this.paramsFilter.ToDueDate
      ...
      : this.http
          .getData('/PreventiveMaintenance/CalenderList', {
            LocationId: localStorage.getItem('defaultLocation'),
            ...Filter,
            ...this.paramsFilter,
            CurrentPage: this.selectedCalenderPage,
            FromDate: this.paramsFilter.FromDate
              ? this.paramsFilter.FromDate
              : this.FromDueDateForCalender,

            ToDate: this.paramsFilter.ToDate
  ...
  TC(body: any) {
    return this.http.saveFormDate('/AppWorkOrders/TechnicalCompleteTask', {
      ...body,
      LocationId: localStorage.getItem('defaultLocation'),
      Lan: this.LocationForUser.coords.longitude,
      Lat: this.LocationForUser.coords.latitude,
    });
  }
  ReOpen(body: any) {
  ...
  deleteSparePart(ID: number, UID: any) {
    return this.http.deleteDate('/WorkOrder/Parts', {
      PartId: ID,
      WorkOrderId: this.WorkOrderOpened.ID,
      UID: UID,
    });
  ...
  PrintWorkOrder(body: any) {
    return this.http.getBlob(
      this.isShedule?.value
        ? '/AllReports/PMReportPrint'
        : '/AllReports/PPMWOPrint',
      {
  ...
  Transfer(body: any) {
    return this.http.saveData('/CoreJobs/RunPPMPeriods', {
      ...body,

      LocationId: localStorage.getItem('defaultLocation'),
    });
  }
  getAllReportsWOSOPPrint(body: any) {
    return this.http.getBlob('/AllReports/WOSOPPrint', {
      ...body,
      LocationId: localStorage.getItem('defaultLocation'),
    });
  }
