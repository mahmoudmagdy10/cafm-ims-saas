  ...
  implements OnInit, AfterViewInit, OnDestroy
{
    ...
    this.FormTask = this.fb.group(
      {
        ...
        Parts_Cost: [{ value: '', disabled: true }],
        Emp_Cost: [{ value: '', disabled: true }],
      },
      { updateOn: 'blur' }
    );
  }
  ...
  ngOnInit(): void {
    this.changeStatusCard$ = this.service.changeStatusCard$.pipe(
      tap((value) => {
        ...
        this.accept.patchValue(value.ClientUserAcceptanceDoneByUserID);
      })
    );
    ...
    const Save$ = this.FormTask.valueChanges.pipe(
      switchMap((value: any) => {
        return this.service.addNewTask({
          ...value,
          TaskTypeId: this.FormTask.get('TaskTypeId')?.value || 134,
  ...
  ReturnDataToForm() {
    ...
    if (
      this.dataEdit.TaskStatusId == 3 ||
      this.dataEdit.TaskStatusId == 4 ||
      this.preview
    ) {
      this.FormTask.disable();
      this.PriorityId.disable();
  ...
  saveClientAcceptance() {
    this.service
      .saveClientAcceptance({
        ...
        accept: this.ClientAcceptanceValue,
      })
      .subscribe();
  }
}
