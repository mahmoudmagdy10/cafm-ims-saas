...
export class LayoutService {
  ...
  getLangByUser() {
    this.http.getData('/AppDashboard/UserLang').subscribe((value) => {
      if (value?.userLang?.toLowerCase() != localStorage.getItem('language')!) {
        this.translationService.setLanguage(
          value?.userLang?.languageCode?.toLowerCase()
        );
      }
    });
  }

  logoPath = new BehaviorSubject<any>(undefined);
  logoPath$ = this.logoPath.asObservable();
  getLogo() {
    this.http.getData('/Location/CompanyLogo').subscribe((Logo) => {
      this.logoPath.next(Logo?.ImagePath);
      localStorage.setItem('LogoImagePath', Logo?.ImagePath);
    });
  }
}
