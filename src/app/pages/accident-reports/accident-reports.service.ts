...
export class AccidentReportsService {
  ...
  addIncidentReports(body: any) {
    return this.http.saveData('/IncidentReports', {
      ...body,
      LocationId: localStorage.getItem('defaultLocation'),
    });
  }
  ...
  deleteIncidentReports(ID: any) {
    ...
    return this.http.deleteDate('/IncidentReports', {
      ID: ID,
    });
  }

  getDataForExcel(params?: any) {
    return this.http.getData('/IncidentReports', {
      ...params,
      RowCount: 2002153,
      LocationId: localStorage.getItem('defaultLocation'),
    });
